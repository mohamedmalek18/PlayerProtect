      ...
      <div style={styles.card}>
        ...
        <div style={styles.tabs}>
          <button style={{ ...styles.tab, ...(!isRegister ? styles.activeTab : {}) }}
            onClick={() => setIsRegister(false)}>🔐 Connexion</button>
          <button style={{ ...styles.tab, ...(isRegister ? styles.activeTab : {}) }}
            onClick={() => setIsRegister(true)}>📝 Inscription</button>
        </div>
          ...
          {isRegister && (
            <>
              <div style={styles.row}>
                <input style={styles.input} placeholder="👤 Prénom" value={form.first_name}
                  onChange={(e) => setForm({ ...form, first_name: e.target.value })} />
                <input style={styles.input} placeholder="👤 Nom" value={form.last_name}
                  onChange={(e) => setForm({ ...form, last_name: e.target.value })} />
              </div>
              <input style={styles.inputFull} placeholder="🏟️ Club / Équipe" value={form.club}
                onChange={(e) => setForm({ ...form, club: e.target.value })} />
              <input style={styles.inputFull} placeholder="👤 Nom d'utilisateur" value={form.username}
                onChange={(e) => setForm({ ...form, username: e.target.value })} />
            </>
          )}
          <input style={styles.inputFull} placeholder="📧 Email" type="email" value={form.email}
            onChange={(e) => setForm({ ...form, email: e.target.value })} />
          <input style={styles.inputFull} placeholder="🔒 Mot de passe" type="password" value={form.password}
...
const styles = {
  ...
  input: { flex: 1, background: "#0f0f1a", border: "1px solid #333", color: "#fff", padding: "12px", borderRadius: "8px", fontSize: "14px" },
  inputFull: { background: "#0f0f1a", border: "1px solid #333", color: "#fff", padding: "12px", borderRadius: "8px", fontSize: "14px" },
